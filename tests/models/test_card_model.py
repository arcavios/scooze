...
def test_cardmodel_from_card_instant(json_ancestral_recall, legalities_ancestral_recall):
...
def test_cardmodel_from_card_creature(json_mystic_snake):
...
def test_cardmodel_from_card_token(json_snake_token, legalities_token):
...
def test_fullcardmodel_from_json_watermark(json_anaconda_7ed_foil):
    model = FullCardModel.model_validate(json_anaconda_7ed_foil)
    assert model.watermark == "wotc"


def test_fullcardmodel_from_json_non_english(json_python_spanish):
    model = FullCardModel.model_validate(json_python_spanish)
    assert model.printed_name == "Pitón"


def test_fullcardmodel_from_json_flavor(json_elessar_the_elfstone):
...
def test_fullcardmodel_from_json_attraction(json_trash_bin):
    model = FullCardModel.model_validate(json_trash_bin)
    assert model.attraction_lights == {2, 6}


def test_fullcardmodel_from_json_variation(json_anaconda_portal):
...
def test_fullcardmodel_from_card_instant(json_ancestral_recall, legalities_ancestral_recall):
    card = FullCard.from_json(json_ancestral_recall)
    model = FullCardModel.model_validate(card.__dict__)
    assert model.all_parts is None
    assert model.arena_id is None
    assert model.artist == "Ryan Pancoast"
    assert model.artist_ids == ["89cc9475-dda2-4d13-bf88-54b92867a25c"]
...
def test_fullcardmodel_from_card_transform_planeswalker(
    json_arlinn_the_packs_hope, oracle_arlinn_the_packs_hope, oracle_arlinn_the_moons_fury
):
...
def test_fullcardmodel_from_card_reversible(
    json_zndrsplt_eye_of_wisdom, legalities_zndrsplt_eye_of_wisdom, oracle_zndrsplt_eye_of_wisdom
):
...
def test_fullcardmodel_from_card_watermark(json_anaconda_7ed_foil):
    card = FullCard.from_json(json_anaconda_7ed_foil)
    model = FullCardModel.model_validate(card.__dict__)
    assert model.watermark == "wotc"


def test_fullcardmodel_from_card_non_english(json_python_spanish):
    card = FullCard.from_json(json_python_spanish)
    model = FullCardModel.model_validate(card.__dict__)
    assert model.printed_name == "Pitón"


def test_fullcardmodel_from_card_flavor(json_elessar_the_elfstone):
...
def test_fullcardmodel_from_card_attraction(json_trash_bin):
    card = FullCard.from_json(json_trash_bin)
    model = FullCardModel.model_validate(card.__dict__)
    assert model.attraction_lights == {2, 6}


def test_fullcardmodel_from_card_variation(json_anaconda_portal):
