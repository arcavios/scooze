...
=======
    return Card.model_construct(
            name="Primeval Titan",
            mana_value=6,
>>>>>>> f9019b4 (Use model_construct() instead of model_validate() in all places where validation is not part of the test. Replace pendulum with datetime.)
    )
...
=======
            ...
            Card.model_construct(name="Trail of Crumbs", mana_value=2): 2,
...
def test_format(format):
    deck = Deck.model_construct(format=format)
...
def test_date_played(today):
    deck = Deck.model_construct(datePlayed=today)
...
def test_add_cards_side(side_cards):
    deck = Deck.model_construct(archetype="test_add_cards_side")
