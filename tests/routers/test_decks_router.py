...
class TestDeckRouterWithPopulatedDatabase:
    ...
    async def populate_db(self, deck_model_data: DeckModelData):
        # print(type(deck_model_modern_4c.date_played))
        deck = DeckModel.model_validate(deck_model_data.model_dump(mode="json", by_alias=True))
        print(DeckModel.get_bson_encoders())
        await deck.create()
