...
async def get_card_by_property(property_name: str, value) -> CardModelOut:
    # TODO(#45): router docstrings
    card = await db_core.get_document_by_property(Collection.CARD, property_name, value)
    if card:
...
async def delete_card(id: str) -> CardModelOut:
    # TODO(#45): router docstrings
    deleted_card = await db_core.delete_document(Collection.CARD, id)
...
async def get_cards_random(limit: int) -> list[CardModelOut]:
    # TODO(#45): router docstrings
    cards = await db_core.get_random_documents(Collection.CARD, limit)
    if len(cards) > 0:
...
async def get_cards_by_property(
    ...
    # TODO(#45): router docstrings
    cards = await db_core.get_documents_by_property(Collection.CARD, property_name, items, paginated, page, page_size)
...
async def delete_cards_all() -> int:
    # TODO(#45): router docstrings
    delete_many_result = await db_core.delete_documents(Collection.CARD)
